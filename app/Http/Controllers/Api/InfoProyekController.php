...
class InfoProyekController extends Controller
{
        ...
        $progressProyek = DB::select("
                ...
                LEFT JOIN (
                        SELECT ip.id as id, 
                                ...
                                max(dl.updated_at) as tanggal_dokumentasi,
                                (ip.volume_pekerjaan * ip.harga_satuan) as max_biaya,
                                dl.volume_pekerjaan,
    ...
    public function index()
    {
        ...
        $proyekList = DB::select("
            SELECT ti.*, 
                IFNULL(IF((persen.progress_dana/ti.nilai_kontrak)  >= 1, 
				100, 
                (persen.progress_dana/ti.nilai_kontrak) * 100), 0) as persentase_progress
